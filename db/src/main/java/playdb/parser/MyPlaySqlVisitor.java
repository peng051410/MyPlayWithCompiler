 ...
 */
public class MyPlaySqlVisitor extends SQLiteBaseVisitor<Object> {
    ...
    public Object visitSql_stmt(Sql_stmtContext ctx) {
        ...
        if (ctx.factored_select_stmt() != null &&
                !ctx.factored_select_stmt().select_core().isEmpty()) {
        ...
        if (ctx.K_WHERE() != null) {
            if (!ctx.expr().isEmpty()) {
                ...
                if (o instanceof List) {
                    whereExprs = (List<WhereExpr>) o;
                } else if (o instanceof WhereExpr) {
                    whereExprs = new LinkedList<>();
                    whereExprs.add((WhereExpr) o);
                } else {
                    throw new RuntimeException("visitExpr error");
                }
            }
        }
