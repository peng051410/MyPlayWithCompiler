...
/**
 * 能够表达EBNF的对象。 1.每个GrammarNode可以有多个子节点； 2.子节点之间可以是And关系，或Or关系，由type属性来确定。
 * 3.minTimes和maxTimes属性规定了该节点的重复次数。比如对于+号，minTimes=1，maxTimes=-1，-1代表很多个。
 * 4.该节点可以有名称，也就是词法规则和语法规则中左边的部分。如果没有起名称，系统会根据它的父节点的名称生成自己的缺省名称， 并且以下划线开头。比如_add_Or_1。
 */
public class GrammarNode {
     ...
     * 子节点
     */
    private final List<GrammarNode> children = new LinkedList<>();
    ...
    /**
     * 以文本方式显示Node。显式结果的格式与Antlr的文法格式相同。 对于命名的节点，要把它的子节点都显示出来。 比如：primary节点的：primary: ID | INT_LITERAL | add
     *
     * @return
     */
    ...
    /**
     * 该节点是否是可空的，也就是能否返回Epsilon。算法： 1.如果是Epsilon节点，或者是 *或 ?，可空。 2.And节点：所有子节点都可空； 3.Or节点：任意子节点可空。 4.其他，不为空。
     *
     * @return
     */
