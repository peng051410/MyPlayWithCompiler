...
/**
 * 一个词法分析器。可以根据正则表达式做词法分析。 原理： 1.把正则表达式转换成NFA，再转成DFA。
...
public class Lexer extends Regex {
    ...
    /**
     * 逻辑： 1.找到能消化接下来的字符的DFA； 2.针对这个DFA一直给它发字符，直到不能接受； 3.查看是否是处于结束状态。
     *
     * @param str
     * @param startState
     * @return
    ...
    /**
     * 检查DFAState中的各个NFAstate，看是否是某个词法规则的结束节点。 注意：如果有符合两个词法规则，那么以宣布顺序的先后算。比如关键字和标识符就会重叠。
     *
     * @param state
     * @param root
     * @return
