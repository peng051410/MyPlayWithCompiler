...
/**
 * 进行一些语义检查，包括： 01.break 只能出现在循环语句中，或case语句中；
 * <p>
 * 02.return语句 02-01 函数声明了返回值，就一定要有return语句。除非返回值类型是void。 02-02 类的构造函数里如果用到return，不能带返回值。 02-03 return语句只能出现在函数里。 02-04
 * 返回值类型检查 -> (在TypeChecker里做）
 * <p>
 * 03.左值 03-01 标注左值（不标注就是右值)； 03-02 检查表达式能否生成合格的左值。
 * <p>
