...
public class ASTEvaluator extends PlayScriptBaseVisitor<Object> {
    ...
    /**
     * 栈桢入栈。 其中最重要的任务，是要保证栈桢的parentFrame设置正确。否则， (1)随着栈的变深，查找变量的性能会降低； (2)甚至有可能找错栈桢，比如在递归(直接或间接)的场景下。
     *
     * @param frame
     */
    ...
    private static final class MyLValue implements LValue {
    ...
    /**
     * @param ctx AST Expression ELE Context
     *            ctx.expression()  表达式列表
     *            ctx.expression(0) 左表达式
     *            ctx.expression(1) 右表达式
     *            ctx.bop()         一个TOKEN对象
     *            ctx.ADD()         访问ADD终结符
    ...
    public Object visitExpression(ExpressionContext ctx) {
        ...
        if (ctx.bop != null && ctx.expression().size() >= 2) {
            ...
            //本节点期待的数据类型
            Type type = at.typeOfNode.get(ctx); //数据类型是语义分析的结果
    ...
    /**
     * 根据函数调用的上下文，返回一个FunctionObject。 对于函数类型的变量，这个functionObject是存在变量里的； 对于普通的函数调用，此时创建一个。
     *
     * @param ctx
     * @return
     */
    ...
    /**
     * 对象方法调用。 要先计算完参数的值，然后再添加对象的StackFrame，然后再调用方法。
     *
