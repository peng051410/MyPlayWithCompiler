...
/**
 * 语义解析的第三步：引用消解和类型推断 1.解析所有的本地变量引用、函数调用和类成员引用。 2.类型推断：从下而上推断表达式的类型。 这两件事要放在一起做，因为： (1)对于变量，只有做了消解，才能推断出类型来。
 * (2)对于FunctionCall，只有把参数（表达式)的类型都推断出来，才能匹配到正确的函数（方法)。 (3)表达式里包含FunctionCall,所以要推导表达式的类型，必须知道是哪个Function，从而才能得到返回值。
 */
public class RefResolver extends PlayScriptBaseListener {
    ...
    private boolean firstStatmentInFunction(FunctionDeclarationContext fdx, FunctionCallContext ctx) {
        return fdx.functionBody().block().blockStatements().blockStatement(0).statement() != null
                ...
                &&
                fdx.functionBody().block().blockStatements().blockStatement(0).statement().expression().functionCall()
                        == ctx;
    }
