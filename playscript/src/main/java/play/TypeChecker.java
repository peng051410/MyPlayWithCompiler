...
/**
 * 类型检查。 主要检查: 1.赋值表达式； 2.变量初始化； 3.表达式里的一些运算，比如加减乘除，是否类型匹配； 4.返回值的类型；
 */
                ...
                case PlayScriptParser.URSHIFT_ASSIGN:
                    if (PrimitiveType.isNumeric(type2)) {
                        if (!checkNumericAssign(type2, type1)) {
                            at.log("can not assign " + ctx.expression(1).getText() + " of type " + type2 + " to "
                                    + ctx.expression(0) + " of type " + type1, ctx);
                        }
                    } else {
...
//    }
     ...
     * @param operand2
     */
    private void checkAssign(Type type1, Type type2, ParserRuleContext ctx, ParserRuleContext operand1,
            ParserRuleContext operand2) {
        if (PrimitiveType.isNumeric(type2)) {
            if (!checkNumericAssign(type2, type1)) {
                at.log("can not assign " + operand2.getText() + " of type " + type2 + " to " + operand1.getText()
                        + " of type " + type1, ctx);
            }
    ...
    /**
     * 看一个类型能否赋值成另一个类型，比如： (1) 整型可以转成浮点型； (2) 子类的对象可以赋给父类; (3) 函数赋值，要求签名是一致的。
     *
     * @param from
     * @param to
     * @return
